...
| 0.5.7-draft | 2026-05-03 | **Appendix D: Implementation Status (RFC 6982)**: new appendix listing the running-code implementations of the `environment.*` family. D.1 records Headless Oracle as the reference implementation of `environment.market_state` — endpoints, signing algorithm, coverage (28 global exchanges identified by 25 ISO 10383 MICs and 3 convention MICs for crypto-native venues), reference verifier at github.com/headlessoracle/demo-agent (~150 lines, MIT), architectural rationale at github.com/headlessoracle/essays, x402 settlement infrastructure validation tx 0xeb9da8…b308a on Base mainnet 2026-04-03 (payer wallet self-funded for end-to-end test, not commercial revenue; first external x402 transaction observed 2026-04-17), and the conformance pointer set for §4.1 / §4.2 / §4.6 / §4.7 / §6.5 / §6.8. D.2 records InsumerAPI (Skye Meta Corp, Douglas Borthwick) as the running implementation of the sibling `environment.wallet_state` constraint type per PR #22, sourced from PR #22's property table (ES256 / P-256, kid `insumer-attest-v1`, 1800-second JWT TTL, 33-chain coverage); listed for `environment.*` family completeness, with the canonical Implementation Status appendix for `environment.wallet_state` belonging in that specification. D.3 disclaimer states that listing is not endorsement and verifiers MUST independently verify per §4.2 regardless of Appendix D listing. Per RFC 6982 §2, the appendix is intended to be removed by the RFC Editor before any final standards-track publication; the GitHub history retains it permanently. Document header stays at `0.5-draft`; v0.5.7 is a patch-level refinement adding an informative appendix only — no algorithm changes, no security-model changes, no §4 schema changes, no Headless Oracle code changes required, test count unchanged. Sibling-mirror coordination: a parallel Appendix D on PR #22 is the editor's call on the `environment.wallet_state` side and is expected to land independently on Douglas Borthwick's timing. Co-drafted with Douglas Borthwick. |
| 0.5.6-draft | 2026-04-23 | **PR #23 field-alignment-v2 propagation**: first coordination commit following Mastercard's PR #23 merge into upstream/main (Reda, Apr 20 2026). VCT version suffix `.1` added to `mandate.checkout.open` (4 occurrences across §3 dual-mode prose, §4 Appears In, §5.5 wallet example, §5.5 multi-exchange example) and `mandate.payment.open` (2 occurrences across §3 dual-mode prose and §4 Appears In). Constraint type pluralised: `mandate.checkout.allowed_merchant` → `mandate.checkout.allowed_merchants` (2 occurrences in §5.5 examples). Constraint field generalised: `allowed_merchants` → `allowed` (2 occurrences, paired with the pluralisation). §5.5 Block 3 prose collapsed: `(mandate.*, payment.*)` → `(mandate.*)` reflecting PR #23's move of payment constraints under the `mandate.*` prefix. Additive PR #23 fields (`match_mode`, `constraint_policy`, `risk_data`) are out of scope for this RFC — they belong to transactional constraint types, not the `environment.*` family. Backfilled retroactively at v0.5.8 (the v0.5.6 commit `68f4db4` did not record a row at the time of merge). Document header stays at `0.5-draft`; v0.5.6 is a patch-level refinement. No semantic changes to `environment.market_state`; no algorithm changes; no security-model changes; no Headless Oracle code changes required. |
...
| Coverage | 28 global exchanges across equities, derivatives, and digital-asset venues, identified by ISO 10383 MIC codes where registered (25 venues) and by convention MICs for crypto-native venues without ISO registration (3 venues) |
...
| Property | Value |
|----------|-------|
| Organisation | Skye Meta Corp (insumermodel.com) |
...
| Agent-native key provisioning | `POST /v1/keys/buy` — no auth, wallet is identity, USDC/BTC on-chain payment |
